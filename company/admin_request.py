...
q= """select * from forgot where status='new'"""
cur.execute(q)
rec=cur.fetchall()
...
<head>
    ...
    <title>Existing announcement</title>
...
print("""
            <main class="content px-3 py-2">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12"><br>
                            <h4 style="color: green;">PASSWORD REQUEST FROM EMPLOYEE</h4><br><br>
                          <div class="table" style=" box-shadow: -3px -3px 8px 1px rgb(174, 159, 245);" >
                             <table border='2' class="table table-responsive table-bordered  table-hover" style="background-color:#ebf8f6; ">
                                 <thead style="background-color: rgb(152, 218, 248);">
                                    <tr >
                                        <th style="padding-left:20px; padding-right:20px;">ID</th> <th style="padding-left:50px; padding-right:50px;">FirstName</th> <th style="padding-left:50px; padding-right:50px;">LastName</th> <th style="padding-left:80px; padding-right:80px;" >Email</th> <th style="padding-left:50px; padding-right:50px;">department</th> 
                                        <th style="padding-left:50px; padding-right:50px;">Role</th> <th style="padding-left:40px; padding-right:40px;">userid</th> 
                                        <th style="padding-left:40px; padding-right:40px;">Password</th> <th>Acknowledgement</th>
                                    </tr>
                                </thead>""")
for i in rec:
    print("""
                                <tbody>
                                    ...
                                    <form class="form-group" method="post" enctype="multipart/form-data">
                                         <td> <input type="text" class="form-control" value='%s' name="id" readonly></td><td><input type="text" class="form-control" value='%s' name="fname" readonly></td> <td><input type="text" class="form-control" value='%s' name="lname" readonly></td>
                                         <td><input type="text" class="form-control" value='%s' name="email" readonly> </td><td><input type="text" class="form-control" value='%s' name="dep" readonly></td>
                                          <td><input type="text" class="form-control" value='%s' name="role" readonly> </td>  <td><input type="text" class="form-control" value='%s' name="userid" readonly> </td>
                                        <td><input type="text" class="form-control" value='%s' name="password" readonly></td> <td><input type="submit" class="btn btn-primary" value="Approve" name="approve"><input type="submit" class="btn btn-secondary" value="Reject" name="reject"></td>
                                    </form>
...
ppass=form.getvalue("password")
papprove=form.getvalue("approve")
preject=form.getvalue("reject")

if papprove != None:
    q="""update forgot set status='%s' where id='%s' """%(papprove,pid)

    cur.execute(q)
    con.commit()
    print("""
    <script>
    location.href='admin_request.py' ;
    </script>""")
    ...
    toadd=pemail
    subject="password"
    body="Welcome {} {} userid:{} password:{}".format(pfname,plname,puserid,ppass)
    ...
    print("""<script> alert("password sended") </script>""")


if preject != None:
    q="""update forgot set status='%s' where id='%s' """%(preject,pid)
