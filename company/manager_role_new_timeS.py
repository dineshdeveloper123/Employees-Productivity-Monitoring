...
q="""select * from timesheet where empid='%s' """%(userid)
cur.execute(q)
rec1=cur.fetchall()
dep=rec1[0][3]


q2= """select * from timesheet where role='executive' and department='%s' and status='new'"""%(dep)
cur.execute(q2)
rec2=cur.fetchall()
...
<head>
    ...
    <title>role new timeSheet</title>
...
print("""
            <main class="content px-3 py-2">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12"><br>
                            <h4 style="color: green;">  NEW TIMESHEET FOR EXECUTIVE</h4><br>
                         <hr>
                              ...
                              <thead  style="background-color: rgb(152, 218, 248);">
                                    <tr>
                                        <th>S.NO</th> <th>Name</th> <th>Department</th> <th>Role</th><th> Date</th> <th>Day</th>
                                         <th>Timing</th>  <th>Hours</th><th>Work status</th>  <th>Acknowledgement</th> 
    ...
    print("""
                                <tbody>
                                    ...
                                    <form method="post" enctype="multipart/form-data">
                                        <td><input type="text" value='%s' name='eid' readonly style="border:none; width:30px ; background:transparent ;"></td>  <td>%s</td>  <td>%s</td>  <td>%s</td> <td>%s</td> <td>%s </td>
                                        <td>%s</td>  <td>%s </td> <td>%s</td>
                                        <td><input type="submit" class="btn btn-success" value="approved" name="approved"> <input type="submit" class="btn btn-danger" value="reject" name="reject"> </td>
                                    </form>
                                    </tr>"""%(i[0],i[2],i[3],i[4],i[5],i[6],i[7],i[8],i[9]))
...
papproved=f.getvalue('approved')
preject=f.getvalue('reject')

if papproved != None:
    pid=f.getvalue('eid')
    q="""update timesheet set status='%s' where id='%s' """%(papproved,pid)
    cur.execute(q)
    con.commit()
    print("""<script>
    alert("Approved");
...
if preject != None:
    pid=f.getvalue('eid')
    q="""update timesheet set status='%s' where id='%s' """%(preject,pid)
    cur.execute(q)
    con.commit()
    print("""<script>
    alert("rejected");
